...
(ert-deftest xr-lint-bad-shy-group ()
  (let ((text-quoting-style 'grave))
    (should (equal (xr-lint "\\(:?\\)\\(:?a\\)\\(:?a\\|b\\)")
                   '((2 . "Suspect `:?' at start of grouping")
                     (8 . "Suspect `:?' at start of grouping")
                     (15 . "Suspect `:?' at start of grouping"))))
    (should (equal (xr-lint "\\(:?\\(:?\\)\\)")
                   '((2 . "Suspect `:?' at start of grouping")
                     (6 . "Suspect `:?' at start of grouping"))))
    (should (equal (xr-lint "\\(?::?a:?\\|:?b:?\\)")
                   nil))
    (should (equal (xr-lint "\\(a:?\\|:?b:?\\)")
                   nil))))
